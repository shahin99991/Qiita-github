...
tags:
  ...
  - MicrosoftTeams
private: false
updated_at: "2026-09-02T21:12:36+09:00"
...
### 「それ、WorkIQでよくね？」と思った方へ

ここまで読んで、「Microsoft 365 に WorkIQ があるし、それでよくない？」と思った方もいるかもしれません。WorkIQ は WorkIQ で、組織のナレッジグラフを横断してユーザーの作業文脈を理解する、素晴らしい機能です。ただ、今回作りたかったものとは狙いが少し違いました。

- **Teams の会話の流れでそのままタスクを振れる**: `@メンション`一つでその場のスレッドから直接タスクを依頼でき、別アプリに切り替える必要がありません。WorkIQ を含む多くのナレッジ検索系サービスは専用の画面・チャットに移動して質問し直す前提になっており、この「アプリを変えずに済む」体験は Teams Bot ならではの強みです
- **狙った作業の実行までは踏み込まない**: WorkIQ は「情報を見つけて渡す」「文脈を理解して提案する」ところが主戦場で、GitHub への Issue/PR 作成やヒアリングシートの `.xlsx` 生成のような**具体的な成果物をその場で作り切る**部分は担いません。YJK-AI-Agent は会話の文脈理解から先、「実際に手を動かして完成品を返す」までを一気通貫でやらせたかったのがそもそもの出発点です
- **社内特有のフォーマットに寄せられる**: ヒアリングシートや提案スライドは会社ごとにテンプレートが決まっています。汎用サービスの出力をそのまま使うより、自社フォーマットに合わせた生成ロジックを自前で持つ方が、そのまま業務に流し込めます
- **GitHub 中心のワークフローとの相性**: 開発チームの成果物は最終的に GitHub の Issue・PR・コミットに集約されます。WorkIQ は Microsoft 365 エコシステム内の情報流通が主軸なので、GitHub 操作までを自然にカバーする設計とは前提が異なります
- **PoC としての柔軟性**: 自前実装なら、ツールの追加・プロンプトの調整・モデルの差し替え（今回の Gemini → 将来の Foundry 移行など）を、社内のリリースサイクルに合わせて即座に試せます

つまり「情報へのアクセス・理解」は WorkIQ が強い領域、「その先の実行・成果物生成」は YJK-AI-Agent のような自前エージェントが得意な領域、という棲み分けです。将来的には WorkIQ（または後述の Foundry IQ）をナレッジ層として使いつつ、YJK-AI-Agent 側は実行層に徹する、という組み合わせ構成もあり得ると考えています。
