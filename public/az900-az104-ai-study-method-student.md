...
tags:
  ...
  - 勉強法
private: false
updated_at: '2026-03-08T05:40:23+09:00'
...
:::note info
💡 **出題数の上限**: GeminiとChatGPTで1回あたりの出題数に差があります。
...
章ごとにまとまった問題量を確保したいならGeminiが有利です。
:::
...
![Geminiが出力したUIベース選択問題の例](ここにGeminiの画像URLを貼る)

![ChatGPTが出力したUIベース選択問題の例（色味あり）](ここにChatGPTの画像URLを貼る)

:::note warn
⚠️ **太字禁止の指示が重要な理由**: この指示がないと、AIが**正解の選択肢だけ太字**にして出力することがあります。答えが視覚的にバレてしまうため、問題として成り立ちません。このプロンプトを入れることで、本番試験と同じ「全選択肢が均等な見た目」を再現できます。
:::

:::note info
💡 **「UIベース」を指定する理由**: これを指定しないとAIがMarkdownのリスト形式や独自レイアウトで出力してしまい、本番試験の形式に慣れられません。試験本番は選択肢A〜Dがカード形式で並ぶUIなので、事前にその形式で解く習慣をつけることが大切です。
:::
...
### AZ-104試験中はMicrosoft Learnが使える

これも実際に受験して気づいたことですが、**AZ-104の試験中はMicrosoft Learnのドキュメントを参照できます**（試験画面からブラウザで開ける）。

:::note info
💡 **だから普段からLearnで学ぶのが正解**: 普段の勉強をMicrosoft Learnで行っておくと、試験中に「見慣れたページ」として素早く目的の情報にたどり着けます。逆に参考書だけで勉強していると、いざ試験中に開いてもドキュメントの構成に迷ってしまいます。

学習中から [Microsoft Learn（AZ-104学習パス）](https://learn.microsoft.com/ja-jp/credentials/certifications/azure-administrator/) を使い、**ドキュメントのどこに何が書いてあるかを体で覚えておく**ことをおすすめします。
:::
...
- **AZ-104の勉強はMicrosoft Learnで**：試験中にLearnを参照できるので、普段から使い慣れておくと本番で強い

資格はあくまで通過点ですが、インフラ系の業務に入る前にAZ-104相当の知識があると、現場での理解速度が全然違います。特に「なぜこの設計か」を考える習慣は、資格の勉強中から意識するだけで、エンジニアとしての質が上がると感じています。
