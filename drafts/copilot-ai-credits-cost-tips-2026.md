...
#### コマンドで会話を軽くするTips（使える環境のみ）

一部の Copilot Chat 環境では、スラッシュコマンドで文脈管理ができます。使える場合はコスト最適化に効きます。

- `/compact`：直近の会話を要約して、以降の入力トークンを軽くする
- `/fork`：別スレッドに分岐して、不要な過去文脈を引きずらない
- `/clear`（または新規チャット開始）：文脈をリセットして再スタートする

> ※ コマンドの対応状況はクライアント（GitHub.com / VS Code / 拡張機能のバージョン）で異なります。使えない場合は、手動で新規スレッドを作る運用でも同じ効果を狙えます。
